<template>
  <NeoCard variant="depressed" class="p-8">
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      <div>
        <label class="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-2 px-1">Target Curriculim</label>
        <select v-model="form.course" required class="w-full bg-[var(--neo-bg)] border border-zinc-200 dark:border-zinc-800 rounded-2xl px-5 py-4 text-sm text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-brand/20 transition-all shadow-neo-inner outline-none appearance-none">
          <option value="" disabled>Choose target node</option>
          <option v-for="c in courses" :key="c._id" :value="c._id">{{ c.title }}</option>
        </select>
      </div>
      <div>
        <label class="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-2 px-1">Diagnostic Label</label>
      ...
      <div>
        <label class="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-2 px-1">Temporal Constraint (mins)</label>
